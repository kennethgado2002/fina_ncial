    ...
    <!-- Summary Cards -->
      <div class="summary-cards">
        <div class="card">
          ...
          <div class="card-footer">₱8,420,000.00</div>
        </div>
        <div class="card">
          <div class="card-title">Total Collections</div>
          <div class="card-value">38</div>
          <div class="card-footer">₱8,350,500.00</div>
        </div>
        <div class="card">
          <div class="card-title">Matched Records</div>
          <div class="card-value">795</div>
          <div class="card-footer">
            <span class="status-badge status-matched">94.4%</span>
          </div>
        </div>
        <div class="card">
          ...
          <div class="card-footer">
            <span class="status-badge status-unmatched">5.6%</span>
          </div>
        </div>
      </div>
      ...
      <div class="tabs">
        ...
        <div class="tab" data-tab="collections">COD Collections</div>
    ...
    <div id="manualMatchModal" class="modal-overlay">
      <div class="modal-box">
        ...
        <form id="manualMatchForm">
          ...
          <div class="form-group mb-3">
            <label for="collectionSelect">COD Collection</label>
            <select class="form-control" id="collectionSelect" required>
...
<script>
  // Sample data for payments (reusing from gateway reconciliation)
      let payments = [
        {
          payment_id: 1001,
          customer_id: 20015,
      ...
      // Sample data for COD collections
      let collections = [
        {
          cod_id: 301,
          partner_id: "LBC",
      ...
      // Sample data for bank deposits (reusing from gateway reconciliation)
      let deposits = [
        {
          deposit_id: 801,
          bank_name: "BDO",
      ...
      // Sample data for mismatches (reusing from gateway reconciliation)
      let mismatches = [
        {
          record_id: 901,
          type: "amount",
        ...
        },
        {
          record_id: 902,
          type: "missing",
          description: "Missing bank deposit for NinjaVan collection",
          expected_amount: 3200.00,
          actual_amount: 0.00,
          date: "2025-09-03",
          status: "pending"
      ...
      function formatPaymentStatus(status) {
        switch (status) {
          ...
          case "received":
            return `<span class="status-badge status-received">Received</span>`;
          case "unsettled":
      ...
      function formatCollectionStatus(status) {
        switch (status) {
          ...
          case "received":
            return `<span class="status-badge status-received">Received</span>`;
          case "short":
            return `<span class="status-badge status-short">Short</span>`;
          case "delayed":
            return `<span class="status-badge status-delayed">Delayed</span>`;
          default:
            return status;
        }
      }
      ...
      manualMatchForm.addEventListener("submit", function(e) {
        ...
        if (!paymentId || !collectionId || !depositId) {
        ...
        const collection = collections.find(c => c.cod_id == collectionId);
        const deposit = deposits.find(d => d.deposit_id == depositId);
        
        if (payment) payment.status = "received";
        if (collection) collection.status = "received";
